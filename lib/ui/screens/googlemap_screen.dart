...
//
  ...
  void initState() {
    ...
    if (Hive.box(userDetailBoxKey).get(latitudeKey) != null &&
      ...
      storedLatitude =
          double.parse(Hive.box(userDetailBoxKey).get(latitudeKey).toString());
      storedLongitude =
          double.parse(Hive.box(userDetailBoxKey).get(longitudeKey).toString());
    } else {
      storedLatitude = 0.0;
      storedLongitude = 0.0;
    }
...
//
      ...
      initialCameraPosition = CameraPosition(
          zoom: 16, target: LatLng(storedLatitude, storedLongitude));
      ...
      markerController.sink.add({
        Marker(
          ...
          position: LatLng(double.parse(widget.place!.latitude),
  ...
  Future<void> createAddressFromCordinates(
      final latitude, final longitude) async {
    ...
    lineOneAddress = filterAddressString(
                ...
                children: [
                  GoogleMap(
                    ...
                    onMapCreated: (GoogleMapController controller) async {
                      if (context.read<AppThemeCubit>().state.appTheme ==
                          AppTheme.dark) {
                        await controller.setMapStyle(
                          await rootBundle
                              .loadString("assets/mapTheme/darkMap.json"),
                        );
                        setState(() {});
                      }
