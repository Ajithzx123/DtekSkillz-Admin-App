...
// _scrollController fetches the next paginated data when the current position of the user on the screen has surpassed
      if (widget.scrollController.position.pixels > nextPageTrigger) {
        if (mounted) {
          context
              .read<BookingCubit>()
              .fetchMoreBookingDetails(status: widget.selectedStatus);
        }
      }
    });
  }
  ...
  void fetchBookingDetails() {
    Future.delayed(Duration.zero).then((final value) {
      context
          .read<BookingCubit>()
          .fetchBookingDetails(status: widget.selectedStatus);
    });
  }
  ...
  Widget getBookingDetailsData({
    ...
    print("bookings are ${bookingDetailsList.length}");
    return SingleChildScrollView(
      ...
      child: bookingDetailsList.isEmpty
          ...
          : Column(
              children: List.generate(
                  bookingDetailsList.length + (isLoadingMoreData ? 1 : 0),
                  (final int index) {
                if (index >= bookingDetailsList.length) {
                  return Center(
                    child:
                        _getBookingShimmerLoading(numberOfShimmerContainer: 1),
                  );
                }
